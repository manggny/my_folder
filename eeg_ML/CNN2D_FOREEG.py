...
EEG_CNN = createModel(train_data.shape[1:], num_Classes)
opt = keras.optimizers.Adam(lr = 0.001,decay=1e-6)
EEG_CNN.compile(loss='sparse_categorical_crossentropy',optimizer=opt,metrics=['accuracy'])

history = EEG_CNN.fit(train_data,train_labels_one_hot,batch_size=BATCH_SIZE,epochs=EPOCHS,validation_data=(test_data,test_labels_one_hot),callbacks=[checkpoint])
EEG_CNN.evaluate(test_data, test_labels_one_hot)
