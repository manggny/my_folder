...
dic_file_fa = 'F:/Insula-Gcamp6/record/record_split_by_behav/20%_nogo_fa_trials.pkl'
dic_file_cr = 'F:/Insula-Gcamp6/record/record_split_by_behav/20%_nogo_cr_trials.pkl'
...
for loop in range(loop_time):
	...
	for time in range(ticks):
		...
		for i in idx_data_fa:
			d_fa = data_fa[i, start + time * time_step:start + time_step + time * time_step]
...
plt.ylabel('no-go svm accuracy of each time(0s~1s)')
plt.xlabel('Time(0s~1s,0.5s for each ticks)')
plt.xticks(x_pos)
plt.ylim([0.4,0.8])

plt.title('no-go fa-cr svm accuracy of each time(0.5s)')
plt.savefig(result_path+'0~1s_result_acc_plot of fa-cr_svm_0724 20%.png')