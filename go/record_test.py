...
if __name__=="__main__":
	...
	cue_file = '#5_Lacc(r,2)+rai(b,3)_0122_gonogotest-Event.pkl'
	cal_file = '#5_Lacc(r,2)+rai(b,3)_0122_gonogotest.pkl'
	cuename = path + cue_file
	calname = path + cal_file
	behavname = beha_path + '#5_gca_recordgonogo50%_0122_odor1_2.lvm'
	...
	for i in range(np.alen(odor1_pump[:,1])):
		...
		if np.sum(odor1_lick[i,:])>0 and (np.sum(odor1_pump[i,:]) + np.sum(odor1_airpuff[i,:])) > 40:
			for y in range(len(odor1_pump[i,:])-1):
				if odor1_pump[i,y] == 1:
					p.append(y/2)
					break
	...
	for q in range(len(odor2_nopump_trials[:,k])):
		plt.plot(x1, odor2_nopump_trials[q,:], 'r')
		plt.plot([0, 0], [0, 4], 'k--', linewidth=1, color='blue')
		plt.plot([1, 1], [0, 4], 'k--', linewidth=1, color='blue')
		# plt.plot([(p[q]/50)-2, (p[q]/50)-2], [0, 4], 'k--', linewidth=1, color='red')
		# plt.plot([(p[q] / 50) - 1.5, (p[q] / 50) - 1.5], [0, 4], 'k--', linewidth=1, color='red')
		#print(p[q])
		plt.show()
