...
if __name__ == '__main__':
	path = "F:/ACC-Camk2/0109~gonogo/gonogopkl"#D:\expdata\laser"#F:/gonogodata/nodelay/laser"#F:/gonogodata/nodelay/laser"
	...
	if exist == 0:
		...
		book.save('pkl_result_laser.xls')

	for raw_file in filelist:
		if raw_file == 'pkl_result_laser.xls':
			continue
		oldwb = xlrd.open_workbook('pkl_result_laser.xls')
		newwb = copy(oldwb)
	...
	#	print(name)
		...
		ori = pkl.load(pkl_file)

		odor1_lick = ori['odor1_lick']
		odor2_lick = ori['odor2_lick']
		odor1_action = ori['odor1_action']
		odor1_airpuff = ori['odor1_airpuff']
		odor1_pump = ori['odor1_pump']
		odor1_laser = ori['odor1_laser']
		odor2_action = ori['odor2_action']
		odor2_airpuff = ori['odor2_airpuff']
		odor2_pump = ori['odor2_pump']
		odor2_laser = ori['odor2_laser']
		...
		for tri in range(len(odor1_lick[:, 1])):
			...
			for ms in range(len(odor1_lick[1, :])):
				...
				if (odor1_pump[tri, ms] == 1 or odor1_airpuff[tri, ms] == 1) and (odor1_did == 0):
					if (np.sum(odor1_pump[tri, ms:ms + 100]) + np.sum(odor1_airpuff[tri, ms:ms + 100])) > 10:
		#				print(tri)
						odor1_hit += 1
						odor1_did = 1
					if laser_trial == 1:
		...
		for tri in range(len(odor2_lick[:, 1])):
			...
			for ms in range(len(odor2_lick[1, :])):
				...
				if (odor2_pump[tri, ms] == 1 or odor2_airpuff[tri, ms] == 1) and (odor2_did == 0):
					if (np.sum(odor2_pump[tri, ms:ms + 100]) + np.sum(odor2_airpuff[tri, ms:ms + 100])) >10 :
						odor2_hit += 1
						odor2_did = 1
					if laser_trial == 1:
						laser2_hit += 1
	...
	#	print(raw_file + ":\n" + "odor1 trials : %f\nodor2 trials : %f" % (odor1_trial, odor2_trial))
		...
		if (hit + miss) > 1:
			...
			accuracy = 0
		#print(laser_hit,laser_miss,laser_fa,laser_cr)
		...
		if saved == 0:
			...
			os.remove('pkl_result_laser.xls')
			newwb.save('pkl_result_laser.xls')
		i = 0
