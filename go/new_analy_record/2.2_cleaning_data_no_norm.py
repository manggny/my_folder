...
def opt_cal(cal):
	xdata = np.linspace(0, round(len(cal[3]) / 100), len(cal[3]))
	y_r = cal[2]
	y_l = cal[3]

	if ~np.isnan(y_r[2]):
		plt.plot(xdata, y_r, 'b-')
		popt, pcov = curve_fit(func, xdata, y_r, maxfev=100000)
		y2_r = [func(i, popt[0], popt[1], popt[2]) for i in xdata]
	...
	if ~np.isnan(y_l[2]):
		plt.plot(xdata, y_l, 'b-')
		popt, pcov = curve_fit(func, xdata, y_l, maxfev=100000)
	# print(popt)
		y2_l = [func(i, popt[0], popt[1], popt[2]) for i in xdata]
...
if __name__=="__main__":
	...
	for filename in filelist:
		...
		result_name_1 = name + '_cleaned_r.pkl'
		result_name_2 = name + '_cleaned_l.pkl'

		if (result_name_1 in filelist_result) or (result_name_2 in filelist_result):
		...
		if np.alen(cal_l) > 1:
			cue1_cal_l, cue2_cal_l, all_cue_cal_l, cue_order_l = div_by_cue(cal[0], cal_l, cue[1], cue[2], 1000, delay=2)

		for c in range(len(cue_order_l)):
			if cue_order_l[c]  == odor_list[c]:
				continue
			else:
				print('!!!!! cue is difff!!!!')
